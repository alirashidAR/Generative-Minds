...
class PostProcess:
    ...
    def process(self, input_images, gen_images, model_cycles = 1, advanced = False, border_padding = 2, verbose = 0):
        ...
        for index in range(len(input_images)):
            ...
            outside_area=(input_images[index][:,:,0]==1).astype(int).reshape(256,256,1)
            result=result*outside_area
            outside_walls=((input_images[index][:,:,0]*255!=0)+(input_images[index][:,:,1]*255!=0)+(input_images[index][:,:,2]*255!=0)).reshape(256,256,1)
            result=result*outside_walls
            not_front_door_=(((input_images[index][:,:,0]*255!=255)+(input_images[index][:,:,1]*255!=0)+(input_images[index][:,:,2]*255!=0)).reshape(256,256,1))
            front_door_=(((input_images[index][:,:,0]*255>=240)*(input_images[index][:,:,1]*255==0)*(input_images[index][:,:,2]*255==0)).reshape(256,256,1))
            result=result*not_front_door_+front_door_*(self.color[15].reshape(1,1,3))
            ...
            for i,label_items in enumerate(label_list):
                if i>=13:
                    ...
                    image=self.post_process_image(label_items,advanced,border_padding)
